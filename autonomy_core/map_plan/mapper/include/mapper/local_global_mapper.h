...
class LocalGlobalMapperNode {
  ...
  private:
    ...
    /**
     * @brief Crops the local map from the storage map, transforms it to odometry frame and publishes 
     * @param center_position_map  Robot position in map frame (for sampling the storage map)
     * @param center_position_odom  Robot position in odom frame (to shift the local map)
     */
    void cropLocalMap_(const Eigen::Vector3d &center_position_map,
                  const Eigen::Vector3d &center_position_odom);

    /**
     * @brief Lookup the transform from lidar to map frame and from lidar to odom frame 
    ...
    /**
     * @brief Adds input cloud to storage map, publishes new local map and global map (if enough msgs were received) 
     * @param cloud Input cloud message
     */
    void processCloud_(const sensor_msgs::PointCloud &cloud);

    /**
     * @brief Point Cloud topic callback. Will convert from sensor_msgs::PointCloud2 to sensor_msgs::PointCloud 
     * @param msg Const pointer to input cloud message
     */
    void cloudCallback_(const sensor_msgs::PointCloud2::ConstPtr &msg);

    /**
     * @brief Allocates memory for the global mapper and initializes the arrays for inflation 
     */
    void globalMapInit_();
