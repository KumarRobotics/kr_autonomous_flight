...
private:
  ...
  // action lib
  std::unique_ptr<actionlib::SimpleActionServer<action_planner::PlanTwoPointAction>> local_as_;
  ...
  */
  ...
  /**
  * @brief Call planner after setting planner start and goal and specify params (use jrk, acc or vel)
  */
  void process_goal();
  ...
  */
  bool local_plan_process(const Waypoint3D& start, const Waypoint3D& goal, const planning_ros_msgs::VoxelMap& map);
...
void LocalPlanServer::localMapCB(const planning_ros_msgs::VoxelMap::ConstPtr& msg) {
...
LocalPlanServer::LocalPlanServer(ros::NodeHandle& nh) {
  ...
  local_map_sub_ = nh.subscribe("local_voxel_map", 2, &LocalPlanServer::localMapCB, this);
  ...
  mp_planner_util_->setMaxNum(max_num); // Set maximum allowed expansion, -1 means no limitation
  mp_planner_util_->setU(U);// 2D discretization if false, 3D if true
  mp_planner_util_->setTol(tol_pos, tol_vel, tol_acc); // Tolerance for goal region
...
void LocalPlanServer::process_result(const Trajectory3D& traj, bool solved) {
  ...
  if (solved) {
    ...
    decimal_t endt = goal_->execution_time.toSec(); // execution_time (set in replanner) equals 1.0/replan_rate

    // evaluate trajectory for 5 steps, each step duration equals execution_time, get corresponding waypoints and record in result (result_->p_stop etc.)
    // (evaluate the whole traj if execution_time is not set (i.e. not in replan mode))
    ...
    for(int i = 0 ; i < num_goals; i++) {
      ...
      // check if evaluation is successful, if not, set result->success to be false! (if failure case, a null Waypoint is returned)
      if ((pt_f.pos(0) == 0) && (pt_f.pos(1) == 0) && (pt_f.pos(2) == 0) && (pt_f.vel(0) == 0) && (pt_f.vel(1) == 0) && (pt_f.vel(2) == 0)){
        result_->success = 0;
        ROS_WARN_STREAM("waypoint evaluation failed, set result->success to be false");
      ...
      p_fin.orientation.w = 1,
        p_fin.orientation.z = 0;
    ...
    result_->execution_time = goal_->execution_time; // execution_time (set in replanner) equals 1.0/replan_rate
...
void LocalPlanServer::process_goal() {
  ...
  // instead of using current odometry as start, we use the given start position for consistency between old and new trajectories in replan process
...
bool LocalPlanServer::local_plan_process(const Waypoint3D& start, const Waypoint3D& goal, const planning_ros_msgs::VoxelMap& map) {
...
valid = mp_planner_util_->plan(start, goal); // start and new_goal contain full informaiton about position/velocity/acceleration
if(valid) {
  traj_ = mp_planner_util_->getTraj();
}

// for visualization: publish expanded nodes as a point cloud
sensor_msgs::PointCloud expanded_ps = vec_to_cloud(mp_planner_util_->getExpandedNodes());
