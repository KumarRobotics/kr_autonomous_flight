...
int main(int argc, char** argv) {
  ...
  // Draw ellispoid
  {
    ...
    for (const auto& it : E.sample(num)) line.push_back(point_2d(it(0), it(1)));
  ...
  // Draw polygon
  {
    ...
    for (size_t i = 0; i < vertices.size(); i++) {
      ss +=
          std::to_string(vertices[i](0)) + " " + std::to_string(vertices[i](1));
    ...
    mapper.add(p);
    mapper.map(p,
               "fill-opacity:0.2;fill:rgb(51,51,153);stroke:rgb(51,51,153);"
               "stroke-width:2");
  }
  ...
  // Draw line segment
  {
    ...
    boost::geometry::model::linestring<point_2d> line;
    for (const auto& it : line_segment) line.push_back(point_2d(it(0), it(1)));
    mapper.add(line);
    mapper.map(line,
