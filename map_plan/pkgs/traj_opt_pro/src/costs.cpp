...
SphereCost::SphereCost(const NLTraj &traj, const NLTimes &times, BasisBundlePro &basis,int min_dim,const std::vector<Quat, Eigen::aligned_allocator<Quat> > &charts): traj_(traj), times_(times) {
  // get r3 part
...
//  std::vector<Quat, Eigen::aligned_allocator<Quat> > charts = std::vector<Quat, Eigen::aligned_allocator<Quat> >(segs_,Quat::Identity());
  ...
  for(int j=0;j<segs_;j++) {
    ...
    std::vector<RationalPoly> coe1,coe2;// expressions for first and second variables on sphere
    // chart transition
    Quat dchart;
    if(j>0){
    ...
    for (int i = 0; i <= deg_; i++) {
      if(j>0 && i == 0) {
        ...
        auto phiphi = S2R3Traj::getPhiPhi(dchart, traj.at(0).at(j-1).at(1),traj.at(1).at(j-1).at(1));
        coe1.push_back(phiphi.front());
        coe2.push_back(phiphi.back());
      } else if (j>0 && i == 2){
        //              ScopedTimer tm("dphiphi");
        auto dphiphi = S2R3Traj::getdPhiPhi(dchart, traj.at(0).at(j-1).at(1),traj.at(1).at(j-1).at(1), traj.at(0).at(j-1).at(3),traj.at(1).at(j-1).at(3));
        coe1.push_back(dphiphi.front());
        coe2.push_back(dphiphi.back());
      } else {
        //              ScopedTimer tm("trivial");
        coe1.push_back(RationalPoly(SymbolicPoly(SymbolicPoly::PolyArbitrary(1,SymbolicPoly::PolyPair(traj.at(0).at(j).at(i),1)),1.0)));
        coe2.push_back(RationalPoly(SymbolicPoly(SymbolicPoly::PolyArbitrary(1,SymbolicPoly::PolyPair(traj.at(1).at(j).at(i),1)),1.0)));
      }
    ...
    for(int k=0;k < n; k++){
      ...
      total_parts.push_back(boost::make_shared<NestedS2CostPart>(sphere_parts,dt*pi,dt));
    }
  }
...
//    //        for(auto &pi:grad)
//    //          std::cout << "sphere gradient " << pi.row() << " " << pi.col() << " " << pi.value() << std::endl;
//  }
  return result;
...
ETV SphereCost::hessian() {
  ...
  for(auto &p:cost_parts) {
    ...
    //        for(auto &pi:hess)
    //          std::cout << "sphere hess " << pi.row() << " " << pi.col() << " " << pi.value() << std::endl;
    //        std::cout << "cost hess size " << hess.size() << std::endl;
  }
  return result;
}

S2R3Traj::S2R3Traj(const std::vector<std::pair<MatD, VecD> > &cons, Vec5 testn, Vec5 testf, const  std::vector<Quat, Eigen::aligned_allocator<Quat> > &chartss,const  std::vector<decimal_t> &ds ){
...
void S2R3Traj::allocate_poly(const Vec5 &pn,const Vec5 &pf,const std::vector<decimal_t> &ds) {
    ...
    for (int j = 0; j < seg_; j++) {
      ...
      for (int k = 0; k <= deg_; k++) {
        if (j==0 && k == 0){
          ...
          pi.push_back(spline.at(j-1).at(k+1)); // this is only valid for assumptions
        } else {
...
void S2R3Traj::add_Axb(
    const std::vector<std::pair<MatD, VecD>> &cons, bool r3) {
  ...
  for (auto &pair : cons) {
    ...
    for (int i = 0; i <= deg_; i++) {
      if(j>0 && i == 0) {
        //              ScopedTimer tm("phiphi");
        auto phiphi = S2R3Traj::getPhiPhi(dchart, trajs2.at(0).at(j-1).at(1),trajs2.at(1).at(j-1).at(1));
        coe1.push_back(phiphi.front());
        coe2.push_back(phiphi.back());
      } else if (j>0 && i == 2){
        //              ScopedTimer tm("dphiphi");
        auto dphiphi = S2R3Traj::getdPhiPhi(dchart, trajs2.at(0).at(j-1).at(1),trajs2.at(1).at(j-1).at(1), trajs2.at(0).at(j-1).at(3),trajs2.at(1).at(j-1).at(3));
        coe1.push_back(dphiphi.front());
        coe2.push_back(dphiphi.back());
      } else {
        //              ScopedTimer tm("trivial");
        coe1.push_back(RationalPoly(SymbolicPoly(SymbolicPoly::PolyArbitrary(1,SymbolicPoly::PolyPair(trajs2.at(0).at(j).at(i),1)),1.0)));
        coe2.push_back(RationalPoly(SymbolicPoly(SymbolicPoly::PolyArbitrary(1,SymbolicPoly::PolyPair(trajs2.at(1).at(j).at(i),1)),1.0)));
      }
    }
    std::vector<boost::shared_ptr<NestedExpression> > sphere_parts1,sphere_parts2;
    for(auto &co:coe1)
    ...
    for (int i = 0; i < pair.first.rows(); i++) {
      ...
      for (uint k = 0; k < traj.at(0).at(j).size(); k++) {
        boost::shared_ptr<AxbConstraint> con =
            boost::make_shared<AxbConstraint>(this, j, k, ai_sub, pair.second(i));
//        boost::shared_ptr<AxbConstraint> con =
//            boost::make_shared<AxbConstraint>(this, j, k, ai, pair.second(i));
//        if(r3)
          solver.addConstraint(con);
        boost::shared_ptr<NestedAxbConstraint> con2 =
            boost::make_shared<NestedAxbConstraint>(sphere_parts1,sphere_parts2, j, k, ai, pair.second(i),times.at(j)->getVal());
  ...
  for (int d = dim_; d < 9; d++) {
    for (int j = 0; j < seg_; j++) {
      if(d == dim_) {
        ...
        //            trajd.data.at(d).segs.at(j).coeffs.at(1) = charts.at(j).w();
      } else if(d == dim_ + 1) {
        trajd.data.at(d).segs.at(j).coeffs.at(0) = charts.at(j).x();
        //            trajd.data.at(d).segs.at(j).coeffs.at(1) = charts.at(j).x();
      } else if(d == dim_ + 2) {
        trajd.data.at(d).segs.at(j).coeffs.at(0) = charts.at(j).y();
        //            trajd.data.at(d).segs.at(j).coeffs.at(1) = charts.at(j).y();
      } else if(d == dim_ + 3) {
        trajd.data.at(d).segs.at(j).coeffs.at(0) = charts.at(j).z();
        //            trajd.data.at(d).segs.at(j).coeffs.at(1) = charts.at(j).z();
      }
    }
  }
...
void SphereCost::testGrad() {
  ...
  for(auto p:cost_parts) {
    ...
    if((gradvd-numgrad).norm() > 0.5) {
      ...
      for(auto &gi:grad2)
        std::cout << gi.row() << " " << gi.col() << " " << gi.value() << std::endl;
      std::cout << std::endl;
      for(auto &gi:gradn)
        std::cout << gi.row() << " " << gi.col() << " " << gi.value() << std::endl;
      std::cout << std::endl;
