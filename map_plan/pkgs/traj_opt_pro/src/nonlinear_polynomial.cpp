...
bool RationalPoly::isNull() {
  if(num_.arb_size() == 0 && den_.arb_size() == 0 && std::abs(num_.coeff) < 1e-8 && std::abs(den_.coeff) < 1e-8)
    return true;
  else
    return false;
}
...
//          while(i < pa.size() && pa.at(i).first->getId() < pp.first->getId() ) i++;
//          if(i < pa.size() && pa.at(i).first == pp.first) {
      ...
      for(auto &mon1: part.first) {
        if(mon0.first == mon1.first) {
          if(mon0.second == 0) {
            ...
            if(mon0.second != 1 ) {
              prod*= static_cast<decimal_t>(mon0.second)*std::pow(mon0.first->getVal(),mon0.second-1);
            }
          }
        } else {
          if(mon1.second !=0) {
    ...
    for(auto &mon0: part.first) {
      for(auto &mon1: part.first) {
        ...
        for(auto &mon2: part.first) {
          ...
          if(mon2.first->id == mon0.first->id && mon2.first->id == mon1.first->id) { // one var derrive twice
            if(mon2.second == 0  || mon2.second ==1) {
              prod*=0.0;
            } else {
              prod*= static_cast<decimal_t>(mon2.second*(mon2.second-1))*std::pow(mon2.first->val,mon2.second-2);
            }

          } else if(mon2.first->id == mon0.first->id || mon2.first->id == mon1.first->id) { // current var derrive once, 
            //should be be XOR, but elseif order handles this
              if(mon2.second == 0) {
                prod *= 0.0;
              } else {
                prod*= static_cast<decimal_t>(mon2.second)*std::pow(mon2.first->val,mon2.second-1);
              }
          } else {
    ...
    for(auto &p2: arbitrary_map) {
      ...
      while(i<p1.first.size() && j<p2.first.size()) {
        if(p1.first.at(i).first->val < p2.first.at(j).first->val){
          ...
          pa.push_back(std::make_pair(p1.first.at(i).first,p1.first.at(i).second+p2.first.at(j).second)); 
          i++;
          j++;
        }
      }
...
std::ostream &operator<<(std::ostream &os, const SymbolicPoly &poly) {
  ...
  for(auto &p:poly.poly_map) {
    os << p.second << " * c" << std::get<0>(p.first)->getId() << " t" << std::get<1>(p.first)->getId() << " ^ " << std::get<2>(p.first) << std::endl;
  }
  os << "Quadratic parts:" << std::endl;
  for(auto &p:poly.quad_map) {
    os << p.second << " * c" << std::get<0>(p.first)->getId() << " * c" << std::get<1>(p.first)->getId() << " t" << std::get<2>(p.first)->getId() << " ^ " << std::get<3>(p.first) << std::endl;
  }
...
std::vector<RationalPoly> S2R3Traj::getPhiPhi(const Quat &R2TR1, Variable* v1, Variable* v2) {
  Mat3 m = R2TR1.inverse().matrix();
//  SymbolicPoly::PolyArbitrary pc(1.0);// = SymbolicPoly::PolyArbitrary(1,SymbolicPoly::PolyPair(v1,0) ); // constant term
  SymbolicPoly pv1 = SymbolicPoly( SymbolicPoly::PolyArbitrary(1,SymbolicPoly::PolyPair(v1,1) ) , 1.0);
  SymbolicPoly pv2 = SymbolicPoly( SymbolicPoly::PolyArbitrary(1,SymbolicPoly::PolyPair(v2,1) ), 1.0);
  SymbolicPoly p2v1 = SymbolicPoly( SymbolicPoly::PolyArbitrary(1,SymbolicPoly::PolyPair(v1,2) ), 1.0);
  SymbolicPoly p2v2 = SymbolicPoly( SymbolicPoly::PolyArbitrary(1,SymbolicPoly::PolyPair(v2,2) ), 1.0);

  SymbolicPoly den = SymbolicPoly(m(0,0)+1) + (-2.0*m(1,0))*pv1 + (-2.0*m(2,0))*pv2 + (1.0-m(0,0))*p2v1 + (1.0-m(0,0))*p2v2;
  SymbolicPoly num1 = SymbolicPoly(-m(0,1)) + (2.0*m(1,1))*pv1 + (2.0*m(2,1))*pv2 + (m(0,1))*p2v1 + (m(0,1))*p2v2;
  SymbolicPoly num2 = SymbolicPoly(-m(0,2)) + (2.0*m(1,2))*pv1 + (2.0*m(2,2))*pv2 + (m(0,2))*p2v1 + (m(0,2))*p2v2;
...
std::vector<RationalPoly> S2R3Traj::getdPhiPhi(const Quat &R2TR1, Variable *v1, Variable *v2, Variable *dv1, Variable *dv2) {
...
//      std::cout << "dpp part den " << rden << std::endl;
      SymbolicPoly dvj = SymbolicPoly(SymbolicPoly::PolyArbitrary(1,SymbolicPoly::PolyPair(dv[j],1)),1.0);
...
RationalPoly::RationalPoly(const SymbolicPoly &num, const SymbolicPoly &den):num_(num),den_(den){
...
//      decimal_t dgn = hnum_map[in]*den + gnum_map[i]*gden_map[j] - hden_map[in]*num - gden_map[i]*gnum_map[j];
