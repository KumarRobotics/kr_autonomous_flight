...
class SO3CmdToMavros : public nodelet::Nodelet {
 ...
 private:
  void so3_cmd_callback(const SO3Command::ConstPtr &msg);
...
void SO3CmdToMavros::imu_callback(const sensor_msgs::Imu::ConstPtr &pose) {
  ...
  if (so3_cmd_set_ &&
    ...
    const auto last_so3_cmd_ptr = boost::make_shared<SO3Command>(last_so3_cmd_);
...
void SO3CmdToMavros::so3_cmd_callback(const SO3Command::ConstPtr &msg) {
