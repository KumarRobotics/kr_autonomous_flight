...
class Projector {
 public:
  ...
  bool project(const Vec3f &pt) {
    ...
    // A: Ellipsoid is the largest ball centered at the curr_pos (pt) that does not intersect any obs_ (points in point cloud)
    ...
    if ((path_.front() - pt).norm() < 0.5) // directly replace if the distance is less than a threshold (time delay from when the path is planned to now leads to this distance)
    {
    ...
    {
      path = path_;
      path.insert(path.begin(), pt); // in this case, only change path variable locally to avoid messing up path_.
    }
