...
public:
  // Initialize, Activate, Deactivate, update functions are all called by trackers_manager.cpp
  // Goal will have information about trajectory (goal->traj), epoch (goal->epoch), replan_rate (goal->replan_rate) and bloack (goal->block)
   ...
   * Part1: check existance of current traj and next traj, check continuity in position and velocity of current traj and new traj
   * Part2: evaluate trajectory and get the cmd
   * @param use_lambda_ if true and if trajectory dimension is not 9, will check difference in position between odometry and trajectory is larger than pos_err_max_, and (2) the position difference is along the trajectory velocity direction (i.e. odom ahead of trajectory)
   * @param use_yaw_ if true and if trajectory dimension is not 9, will check yaw. If both use_lambda_ and use_yaw_ set as false, will check nothing
   */
...
private:
  ...
  // action lib
  std::unique_ptr<
      actionlib::SimpleActionServer<action_trackers::RunTrajectoryAction>> as_;
  bool prempted{false};
...
ActionTrajectoryTracker::update(const nav_msgs::Odometry::ConstPtr &msg) {
  ...
  //ROS_ERROR_STREAM_THROTTLE(1,"Trajectory tracker active with current_trajectory_" << (current_trajectory_ == NULL));
  //ROS_ERROR_STREAM_THROTTLE(1,"Trajectory tracker active with next_trajectory_" << next_trajectory_.size() << " init_cmd " << (init_cmd_ == NULL) << " epoch size " << traj_epoch.size());
  ...
  if(current_trajectory_ == NULL && next_trajectory_.size() > 0 ) {
    ...
    if ((pos_old - pos_new).norm() > 1.0) {
      ...
      // incontinuity is within threshold, accept next_trajectory_ as new trajectory
      current_trajectory_ = next_trajectory_.front();
      next_trajectory_.erase(next_trajectory_.begin());
      current_epoch_ = traj_epoch.front();
      traj_epoch.erase(traj_epoch.begin());
      state_lambda(0)=0;
      ROS_INFO_STREAM("Accepted new trajectory. Queue sizes: "<< next_trajectory_.size() << " and " << traj_epoch.size() );
    }
  } 
  // both current traj and next traj exist
    ...
    if(duration >= check_time) { 
      ...
      if ((pos_old - pos_new).norm() > 1.0) {
        ...
        ROS_ERROR_STREAM("current epoch: " << current_epoch_ << " traj epoch: " << traj_epoch.front() << " execution time: " << execution_time << " check time " << check_time);
        next_trajectory_.erase(next_trajectory_.begin());
        traj_epoch.erase(traj_epoch.begin());

      }else if ((vel_old - vel_new).norm() > 1.0) {
        // incontinuity in velocity is too large, abort
        ROS_ERROR_STREAM("Replan velocity not continuous from " << vel_old.transpose()<<" to " << vel_new.transpose());
        ROS_ERROR_STREAM("current epoch: " << current_epoch_ << " traj epoch: " << traj_epoch.front() << " execution time: " << execution_time);
        next_trajectory_.erase(next_trajectory_.begin());
        traj_epoch.erase(traj_epoch.begin());

      } else{
        // incontinuity in vel and position are both within threshold, accept next_trajectory_ as new trajectory
        current_trajectory_ = next_trajectory_.front();
        next_trajectory_.erase(next_trajectory_.begin());
        current_epoch_ = traj_epoch.front();
        traj_epoch.erase(traj_epoch.begin());
        ...
        duration = 0.0;
        state_lambda(0)=0;
        // ROS_INFO_STREAM("Accepted replan Queue sizes: "<< next_trajectory_.size() << " and " << traj_epoch.size() );
      }
    }
  }
  ...
  // evaluate trajectory and get the cmd, refer to convert.cpp in traj_opt_quadrotor, 
  if(current_trajectory_->getDim() == 9) { // TODO: what does this indicate?
    ...
    TrajToQuadCmd::evaluateTagentYaw(current_trajectory_,duration,cmd,init_cmd_->yaw,yaw_speed_,0.01,yaw_thr_);
  } else if(use_lambda_) {
    // evaluatePos will return false if difference in position between odometry and trajectory is larger than pos_err_max_
    if(!TrajToQuadCmd::evaluatePos(current_trajectory_,msg,pos_err_max_,duration,0.01, cmd)) {
      // started_ = started_ + ros::Duration(0.01); // this slowing down strategy is not working properly, commented out
      ROS_ERROR("[Traj Tracker:] Max position error threshold violated!! Slow down the execution!!!");
    }
  } else {
    // no pos or yaw check
    TrajToQuadCmd::evaluate(current_trajectory_,duration,cmd,2);
  }
  ...
  if (duration > current_trajectory_->getTotalTime()) {
    ...
    ROS_INFO_STREAM("Null " << (next_trajectory_.size() == 0) << " current epoch " << current_epoch_ << " traj epoch " << traj_epoch.front() );
    init_cmd_->header.stamp = ros::Time::now();
    done_ = true;
    current_trajectory_ = boost::shared_ptr<traj_opt::Trajectory>();
...
void ActionTrajectoryTracker::trajCB() {
  ...
  boost::mutex::scoped_lock lock(mtx_);
  // double sec = t.toc();  // return time between Timer initialization and this point
  // if(sec > 0.2)
    // ROS_ERROR_STREAM("[Traj Tracker:] thread blocked for " << sec);
  ...
  traj_opt_msgs::Trajectory msg = goal->traj; // extract trajectory msg from goal->traj
  boost::shared_ptr<traj_opt::Trajectory> sp = boost::make_shared<traj_opt::MsgTrajectory>(TrajRosBridge::convert(msg));
