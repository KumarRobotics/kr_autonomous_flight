...
class TrajOptUtils{
    public:
        ...
        static void vec_to_pose(const traj_opt::VecD &valn, geometry_msgs::Pose *pos){
            if(pos != NULL) {
                traj_opt::VecD val = make4d(valn);
                pos->position.x = val(0),pos->position.y = val(1),pos->position.z = val(2);
                pos->orientation.w = std::cos(0.5*val(3));
                pos->orientation.z = std::sin(0.5*val(3));
            }
        }
        static void vec_to_twist(const traj_opt::VecD &valn, geometry_msgs::Twist *vel){
            if(vel != NULL) { 
                traj_opt::VecD val = make4d(valn);
        ...
        static void evaluate_to_msgs(boost::shared_ptr<traj_opt::Trajectory> traj, double dt,
