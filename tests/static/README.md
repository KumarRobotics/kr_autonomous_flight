...
| **R** | For every `PathJoinSubstitution([FindPackageShare('<managed>'), 'seg', ...])` in a `*.launch.py`, the resolved path exists in `<managed>`'s source tree. Catches launch files whose config / rviz / sub-launch path references went stale during the port. External packages (not in the 22-package managed map) are skipped; directory-only targets (no extension) are skipped; `msckf_calib.yaml` is carved out because the upstream workflow generates it at first run. |
